...
vim.api.nvim_create_autocmd("BufWritePre", {
	...
	callback = function()
		if vim.bo.filetype == "markdown" then
			return
		end -- markdown uses trailing spaces for line breaks
...
-- auto-reload files changed outside nvim (e.g. by ai tools in another pane)
vim.api.nvim_create_autocmd({ "FocusGained", "BufEnter", "CursorHold", "CursorHoldI" }, {
	group = augroup("auto_reload"),
	callback = function()
		-- only check if the buffer is a normal file and nvim isn't in command mode
...
-- notify when a file was reloaded because it changed on disk
